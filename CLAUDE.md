...
- A `post_save` signal on `Project` seeds the default Kanban columns (Backlog / Todo / In Progress / Waiting / In Review / Done / Cancelled). `Column.is_done=True` on Done is how analytics/recurring defaults find the "completed" column. `ColumnKind.WAITING` means "the ball is in someone else's court" — it is deliberately *not* `IN_PROGRESS`, so analytics never counts it as started, and it ships with a 3/7-day stale threshold since a silent counterparty is the thing you want flagged.
...
- MCP: 14 meeting tools (`upsert_meeting`, `search_meetings`, `get_meeting`, entity merge, …). The wrappers + agent-facing docstrings are in `mcp_server/server.py`; the implementations are in **`apps/meetings/mcp_tools.py`**, which adds no logic of its own — reads go through `query.py`/`related.py`, writes through `services.py`, validation through the same serializers as REST. It only translates the flat argument list (`people=[…]`, `companies=[…]`) and accepts key-or-stem / name-or-id references. `get_meeting` omits the transcript unless asked (it's huge). Pipeline contract: `docs/meetings-pipeline.md`.
